...
import { LayoutDashboard, LogOut, PlusCircle, ReceiptText, Settings, Users, BarChart3 } from "lucide-react";
      ...
      <main className="lg:pl-72">
        <div className="border-b border-slate-200 bg-white p-4 lg:hidden">
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
            {nav.map((item) => (
              <Link key={item.href} href={item.href} className="whitespace-nowrap rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium">
                {item.label}
              </Link>
            ))}
          </div>
